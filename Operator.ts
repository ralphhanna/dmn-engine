...
import { Options, EXPRESSION_TYPE } from "./common";
...
export class Operator {
    ...
    leftOperands;
    rightOperands;
    conditionOperand; // L /R or nothing 
    function;
    ...
    parseOperator(rootNode,parser,forCondition) {
        ...
        list.forEach(node => {
                ...
                if ((node.type == EXPRESSION_TYPE.Operator ||
                    ...
                    if (node.type == EXPRESSION_TYPE.Text)
                        node.type = EXPRESSION_TYPE.Operator;
                    if (leftOperands == 1) {
                        if (conditionOperand == 'L') {
                            const newNode=new ExpressionNode(EXPRESSION_TYPE.Text, node, '$$VALUE', node.position);
                    ...
                    if (rightOperands == 1) {
                        if (conditionOperand == 'R') {
                            new ExpressionNode(EXPRESSION_TYPE.Text, node, '$$VALUE', node.position);
                        }
                        else {

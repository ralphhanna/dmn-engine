...
function testCond(expr, value, res) {
    ...
    if (ret !== res) {
        ...
        logger.log("***Error -" + expr + " for " + value + "result: " + ret + "expecting:" + res);
...
function testExp(expr,  res) {
    ...
    if (ret !== res) {
        ...
        logger.log("***Error -" + expr + "result: " + ret + "expecting:" + res);
    ...
    var conditions=
        ...
        category: {
            ...
            conditions: [
                ...
                ["> 200 or < 500", 20, true],
                ["(> 200) or (< 500)", 20, true],
                ["(in ('abc','xyz-abc')", 'abc', true],    //  not , in, 'abc', 'xyz-abc'
                ["(in ('abc','xyz-abc') ) and startsWith('a')", 'abc', true],    //  not , in, 'abc', 'xyz-abc'

                ["in ('abc','xyz-abc','another entry', 9)", "abc", true],

                ["(2+3)", '5', true],    //  not , in, 'abc', 'xyz-abc'
                ["2+3", '5', true],    //  not , in, 'abc', 'xyz-abc'
                ["2*4", '8', true],    //  not , in, 'abc', 'xyz-abc'


                ["(2+3)*4", '20', true],    //  not , in, 'abc', 'xyz-abc'
                ...
                ["in (abc,'xyz-abc') and startsWith('a') ", 'abc', true],    //  not , in, 'abc', 'xyz-abc'
                ["in (abc,'xyz-abc') and not startsWith('a') ", 'abc', false],    //  not , in, 'abc', 'xyz-abc'
                ["in (abc,'xyz-abc') and startsWith('a')  and 3 ", 'abc', false],    //  not , in, 'abc', 'xyz-abc'
                ["75", 'not 75', false],
                ["'High'", 'High', true],
                ["in (abc,'xyz-abc','another entry')", "abc", true],

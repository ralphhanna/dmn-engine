...
function processScenario(cmds) {
    cmds.forEach(cmd => {
        try {
            ...
            const expr = ExpressionNode_1.Expression.compile(cmd.expr);
            ...
            console.log(exc);
            cmd.errors.push(exc.description);
        }
    });
}
...
function testCond(expr, value, res) {
    ...
    const ret = expr.evaluateCondition(data, value);
    expr.display();
...
function getTokensMask(expr) {
    let tokens = ExpressionParser_1.Parser.tokenize(expr);
    let mask = '';
...
//# sourceMappingURL=Tester2.js.map